...
// Use the environment variable to limit how many threads get created, or default to
// one fewer than the number of CPUs, so that it doesn't slug the whole system.
const cpuCount =
  process.env.ESLINT_CPU_COUNT != null
    ? Number(process.env.ESLINT_CPU_COUNT)
    : os.cpus().length - 1;
